...
/-!
Adapted from Problem56.NestedCoupling, retaining its private construction and
generalizing the exported theorem to arbitrary ordered densities in [0,1].
...
theorem sampling_nested_coupling (n k : ℕ) (θminus θplus : ℝ)
    (hk : k ≤ n) (hθminus : 0 ≤ θminus) (hθle : θminus ≤ θplus)
    (hθplus : θplus ≤ 1) :
    ∃ coupling : FixedSubset (Fin n) k × SignLayer (Fin n) ×
        SignLayer (Fin n) → ℝ,
