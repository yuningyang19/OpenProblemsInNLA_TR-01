...
/-!
# V7 full-density signed-trace estimate

Adapted from `Problem56/SignedTraceExpansion.lean` without modifying v6.
The namespace-local copy retains the private finite-expansion proof chain.
The only mathematical change is replacing the density half-bound by `θ < 1`;
the centered-moment estimate and reciprocal decay need only `θ ≤ 1`.
All graph principles, cumulant bounds, constants and remaining hypotheses agree
...
private theorem selectorMoment_abs_le
    {iota alpha : Type*} [Fintype iota] [DecidableEq iota]
    [Fintype alpha] [DecidableEq alpha]
    (theta : ℝ) (hzero : 0 < theta) (hone : theta < 1)
    (P : Finpartition (Finset.univ : Finset iota))
    (lab : InjectivePartitionLabeling P alpha)
